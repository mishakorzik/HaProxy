...
public final class ProxyProtocolDecoder extends ByteToMessageDecoder {
    ...
    public static final AttributeKey<InetSocketAddress> REAL_ADDRESS =
            AttributeKey.valueOf(ProxyProtocolDecoder.class, "real_address");
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
        ...
        if (matchesAt(in, 0, V2_SIG)) {
            if (config.getVersion() != 2) {
                logger.warning("Expected PROXY v" + config.getVersion() + " but received v2 header from " + ctx.channel().remoteAddress());
        ...
        if (matchesAt(in, 0, V1_PREFIX)) {
            if (config.getVersion() != 1) {
                logger.warning("Expected PROXY v" + config.getVersion() + " but received v1 header from " + ctx.channel().remoteAddress());
                closeOrPassthrough(ctx, in, out);
                return;
            }
            parseV1(ctx, in, out);
        ...
        if (config.isRequireProxyHeader()) {
            logger.warning("No PROXY header from " + ctx.channel().remoteAddress() + " - closing");
            discard(ctx);
        } else {
    ...
    private void parseV1(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
        ...
        if (crlfOffset == -1) {
            if (in.readableBytes() >= 108) {
                logger.warning("PROXY v1 header exceeds 108 bytes from " + ctx.channel().remoteAddress() + " — closing");
                discard(ctx);
            }
            return;
        }
        ...
        try {
            ...
            logger.warning("Malformed PROXY v1 header from " + ctx.channel().remoteAddress() + ": " + e.getMessage());
            if (config.isRequireProxyHeader()) {
                discard(ctx);
                return;
            }
    ...
    private void parseV2(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
        ...
        if (in.readableBytes() < 16 + addrLen) return;
        ...
        if (ver != 2) {
            logger.warning("Invalid PROXY v2 version nibble " + ver + " from " + ctx.channel().remoteAddress());
        ...
        if (cmd == 0x01) {
            try {
                ...
                logger.warning("[HaProxy] Malformed PROXY v2 address block from "
            ...
            logger.warning("Unknown PROXY v2 command 0x" + Integer.toHexString(cmd) + " from " + ctx.channel().remoteAddress());
            if (config.isRequireProxyHeader()) {
                in.readerIndex(headerEnd);
                discard(ctx);
                return;
    ...
    private void injectIntoNmsConnection(ChannelHandlerContext ctx, SocketAddress address) {
        ...
        try {
            ...
            logger.fine("Early address injection deferred: " + e.getMessage());
        }
    }
